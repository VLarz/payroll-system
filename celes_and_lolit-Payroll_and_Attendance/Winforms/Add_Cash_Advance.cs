    ...
    public partial class Add_Cash_Advance : Form
    {
        ...
        public Add_Cash_Advance()
        {
            ...
            if (ID != "")
            {
                ...
                lblTitle.Text = "Update Cash Advance";
            }
        }
        ...
        {
            //if(txtName.Text != "" && txtMin.Text != "" && txtMax.Text != "")
            // {
            //    try
            //    {
            //        conn.Open();
            //        MySqlCommand scom = conn.CreateCommand();
            //        scom.CommandText = "INSERT INTO loan (name, description, minimum_loan, maximum_loan, interest)" +
            //                           "VALUES (@name, @descrip, @min, @max, @interest";
            //        scom.Parameters.AddWithValue("@name", txtName.Text);
            //        scom.Parameters.AddWithValue("@descrip", txtDescription.Text);
            //        scom.Parameters.AddWithValue("@min", txtMin.Text);
            //        scom.Parameters.AddWithValue("@max", txtMax.Text);
        ...
        private void btnSave_Click_1(object sender, EventArgs e)
        {
            if (cmbEmployeeID.SelectedIndex != 0 && txtName.Text != "" && txtAmount.Text != "" && txtDateEnd.Text != "" && txtWeeks.Text != "" & lblPerWeek.Text != "0" && txtInterest.Text != "")
            {
                ...
                scom1.CommandText = "SELECT employee_id FROM cash_advance WHERE employee_id = @id";
                ...
                if (sdr.Read())
                {
                    conn.Close();
                    sdr.Close();
                    alert.Show(txtName.Text + " has already cash advance.", alert.AlertType.warning);
                }
                else
                {
                    conn.Close();
                    ...
                    conn.Open();
                    MySqlCommand scom = conn.CreateCommand();
                    scom.CommandText = "INSERT INTO cash_advance (employee_id, amount, interest, start, week, end, amount_per_week, description) " +
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (cmbEmployeeID.SelectedIndex != 0 && txtName.Text != "" && txtAmount.Text != "" && txtInterest.Text != "" && txtDateEnd.Text != "" && txtWeeks.Text != "" & lblPerWeek.Text != "0")
            {
                ...
                conn.Open();
                MySqlCommand scom = conn.CreateCommand();
                scom.CommandText = "UPDATE cash_advance SET employee_id = @id, amount = @amount, interest = @interest, start = @start, week = @week, end = @end, amount_per_week = @perWeek, description = @description " +
                                   "WHERE id = @getID";
