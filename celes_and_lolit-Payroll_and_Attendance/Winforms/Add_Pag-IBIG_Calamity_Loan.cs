...
{
    public partial class Add_Pag_IBIG_Calamity_Loan : Form
    {
        string ID = "";
        MySqlConnection conn = new MySqlConnection("SERVER = localhost; USERNAME = root; PASSWORD = ; DATABASE = celes_and_lolit-payroll");
        public Add_Pag_IBIG_Calamity_Loan()
        {
            InitializeComponent();
            ID = Pag_IBIG_Loan.CalamityID;
            getEmployeeID();
            if (ID != "")
            {
                btnUpdate.Visible = true;
                getEmployeeInformation();
                lblTitle.Text = "Update Pag-IBIG Calamity Loan";
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (cmbEmployeeID.SelectedIndex != 0 && txtName.Text != "" && txtAmount.Text != "" && txtDateEnd.Text != "" && lblPerMonth.Text != "0" & lblPerWeek.Text != "0")
            {
                ...
                conn.Open();
                MySqlCommand scom = conn.CreateCommand();
                scom.CommandText = "INSERT INTO pagibig_calamity_loan (employee_id, amount, start, end, amount_per_month, amount_per_week) " +
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (cmbEmployeeID.SelectedIndex != 0 && txtName.Text != "" && txtAmount.Text != "" && txtDateEnd.Text != "" && lblPerMonth.Text != "0" & lblPerWeek.Text != "0")
            {
                ...
                conn.Open();
                MySqlCommand scom = conn.CreateCommand();
                scom.CommandText = "UPDATE pagibig_calamity_loan SET employee_id = @employeeID, amount = @amount, start = @start, end = @end, amount_per_month = @perMonth, amount_per_week = @perWeek " +
                                    "WHERE id = @id";
                scom.Parameters.AddWithValue("@id", ID);
